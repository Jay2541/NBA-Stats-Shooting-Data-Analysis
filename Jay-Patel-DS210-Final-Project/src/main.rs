...
use crate::data_loader::{load_players, load_teams};
...
fn main() {
    let player_data = load_players("NBA Stats (1947-Present)/Player Shooting.csv");
    match load_players("NBA Stats (1947-Present)/Player Shooting.csv") {
        Ok(mut players) => {
            for player in players {
                println!("{:?}", player);  // Assuming Player implements Debug
            }
        },
        Err(e) => println!("Failed to load player data: {}", e),
    }
